...
Threshold reached.\n", *my_id, count);
		...
		/* Prevent starvation: Do some work so threads can 
...
void *watch_count(void *idp)
{
	...
	/*
	   ...
	   routine will automatically and atomically unlock the mutex while it 
	   waits. 
...
int main(int argc, char **argv)
{
	...
	/* The first two are the worker threads */
	pthread_create(&threads[0], &attr, inc_count, (void *)&thread_ids[0]);
	pthread_create(&threads[1], &attr, inc_count, (void *)&thread_ids[1]);
	/* The last is the one that will wait upon the condition */

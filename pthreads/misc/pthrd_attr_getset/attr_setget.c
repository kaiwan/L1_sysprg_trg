...
static void display_pthread_attr(pthread_attr_t * attr, char *prefix)
{
	...
	printf("%sStack size          = 0x%x (%u) bytes\n", 
			prefix, (unsigned int)v, (unsigned int)v);
}
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		...
		/* man posix_memalign(3) :
		   ...
		   of the allocated memory in *memptr.   The  address of  the allocated 
		   memory will be a multiple of alignment, which must be a power of two and a 

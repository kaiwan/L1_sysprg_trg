...
{
	printf("thrd_p2: will terminate in 2s...\n");
	sleep (2);
	pthread_exit( NULL );
}
...
void * thrd_p3(void * msg)
{
	...
	if (!fname) {
		fprintf(stderr, "thrd_p3: PDF-file-to-view absent?\n");
		exit(1);
	}
	printf("thrd_p3: PID %d...fname=%s\n", getpid(), fname);
	...
	switch (n=fork()) {
		...
		case 0: // Child
			...
			if (execl ("/usr/bin/evince", "evince PDF reader", fname, (char *)0) < 0) {
				perror("execl failed!");
				exit (1);
			}
			exit (0);
...
int main( int argc, char **argv )
{
	...
	if (((fd=open(argv[1], O_RDONLY)) == -1)) {
		fprintf(stderr, "%s: PDF-file-to-view \"%s\" not existing or unreadable?\n", argv[0], argv[1]);

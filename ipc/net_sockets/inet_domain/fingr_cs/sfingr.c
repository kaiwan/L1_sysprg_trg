...
static int process_client(int sd, char *prg)
{
	...
	if ((n = read(sd, username, 32)) == -1)
		err_exit(prg, "socket read error", 6);
	username[n] = '\0';

	reply = (char *)malloc(REPLY_MAXLEN);
	if (!reply) {
	...
	if (p == NULL) {
		snprintf(reply, 256 + 256,
			 "\n%s is unable to get info for username \"%s\"\n", prg, username);
	...
	snprintf(reply, REPLY_MAXLEN, "\tUsername: %s\n\t(encrypted) password: %s\n\
\tUID: %d\n\tGID: %d\n\tComment/FullName: %s\n\tHome dir: %s\n\tShell: %s\n",
...
int main(int argc, char *argv[])
{
	...
	/* Actually, we can use any valid IPaddr on an interface by specifying it as INADDR_ANY
...
port # %d\n", argv[0], inet_ntoa(cli_addr.sin_addr), ntohs(cli_addr.sin_port));
		...
		case 0:	// Child server
			/* for debugging with GDB, keep it asleep

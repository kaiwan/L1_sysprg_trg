 ...
 * 'Restart' library.
 *
 * All functions below handle signals interrupting system calls, etc.
 * (c) "UNIX Systems Programming", Robbins & Robbins.
 */
...
ssize_t readblock(int fd, void *buf, size_t size)
{
	...
	for (bufp = buf, bytestoread = size, totalbytes = 0;
	     bytestoread > 0; bufp += bytesread, bytestoread -= bytesread) {

...
{
	struct sigaction act;
	// Set up the stack to eat a lot of memory
#define NUMBYTES (7*1024*1024)
#define NUMEL (NUMBYTES/sizeof(long long))
	long long longarr[NUMEL];

	memset(&longarr[0], 0xef, NUMBYTES);
